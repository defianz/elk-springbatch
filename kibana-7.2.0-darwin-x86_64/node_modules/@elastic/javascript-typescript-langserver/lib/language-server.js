...
const logging_1 = require("javascript-typescript-langserver/lib/logging");
const opentracing_1 = require("opentracing");
const extended_typescript_service_1 = require("./extended-typescript-service");
...
class FilteredStdioLogger extends logging_1.StdioLogger {
    warn(...values) {
        if (typeof values[0] === 'string' && values[0].startsWith('TypeScript config file for')) {
            super.info(values);
        }
        else {
            super.warn(values);
...
const options = {
    ...
    logger: program.logfile ? new logging_1.FileLogger(program.logfile) : new FilteredStdioLogger(),
...
server_1.serve(options, client => new extended_typescript_service_1.ExtendedTypescriptService(client, options));
