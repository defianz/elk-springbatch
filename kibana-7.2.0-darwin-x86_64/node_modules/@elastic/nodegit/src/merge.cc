  ...
  // end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (baton->error->message) {
        ...
        err = Nan::Error("Method analysis has thrown an error.")->ToObject();
      }
      err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
      err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("Merge.analysis").ToLocalChecked());
      v8::Local<v8::Value> argv[1] = {
        err
      };
      callback->Call(1, argv, async_resource);
      ...
      if (!callbackFired) {
        v8::Local<v8::Object> err = Nan::Error("Method analysis has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("Merge.analysis").ToLocalChecked());
        v8::Local<v8::Value> argv[1] = {
          err
...
from_our_ref = Nan::ObjectWrap::Unwrap<GitRefs>(info[1]->ToObject())->GetValue();
// end convert_from_v8 block
          baton->our_ref = from_our_ref;
// start convert_from_v8 block
  const git_annotated_commit ** from_their_heads = NULL;

  Array *tmp_their_heads = Array::Cast(*info[2]);
  from_their_heads = (const git_annotated_commit **)malloc(tmp_their_heads->Length() * sizeof(const git_annotated_commit *));
      for (unsigned int i = 0; i < tmp_their_heads->Length(); i++) {
      from_their_heads[i] = Nan::ObjectWrap::Unwrap<GitAnnotatedCommit>(tmp_their_heads->Get(Nan::New(static_cast<double>(i)))->ToObject())->GetValue();
      }
...
// end convert_from_v8 block
  ...
  AnalysisForRefWorker *worker = new AnalysisForRefWorker(baton, callback);
        ...
        if (!info[0]->IsUndefined() && !info[0]->IsNull())
          worker->SaveToPersistent("repo", info[0]->ToObject());
        if (!info[1]->IsUndefined() && !info[1]->IsNull())
          worker->SaveToPersistent("our_ref", info[1]->ToObject());
        if (!info[2]->IsUndefined() && !info[2]->IsNull())
          worker->SaveToPersistent("their_heads", info[2]->ToObject());
        if (!info[3]->IsUndefined() && !info[3]->IsNull())
          worker->SaveToPersistent("their_heads_len", info[3]->ToObject());
  ...
  // end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (baton->error->message) {
        ...
        err = Nan::Error("Method analysisForRef has thrown an error.")->ToObject();
      }
      err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
      err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("Merge.analysisForRef").ToLocalChecked());
      v8::Local<v8::Value> argv[1] = {
        err
      };
      callback->Call(1, argv, async_resource);
      ...
      if (!callbackFired) {
        v8::Local<v8::Object> err = Nan::Error("Method analysisForRef has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("Merge.analysisForRef").ToLocalChecked());
        v8::Local<v8::Value> argv[1] = {
          err
