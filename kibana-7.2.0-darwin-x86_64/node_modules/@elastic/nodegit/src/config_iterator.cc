...
from_cfg = Nan::ObjectWrap::Unwrap<GitConfig>(info[0]->ToObject())->GetValue();
// end convert_from_v8 block
  ...
  CreateWorker *worker = new CreateWorker(baton, callback);
        ...
        if (!info[0]->IsUndefined() && !info[0]->IsNull())
          worker->SaveToPersistent("cfg", info[0]->ToObject());
...
// start convert_to_v8 block
  ...
  // end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("ConfigIterator.create").ToLocalChecked());
  ...
  // end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (baton->error->message) {
        ...
        err = Nan::Error("Method createGlob has thrown an error.")->ToObject();
      }
      err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
      err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("ConfigIterator.createGlob").ToLocalChecked());
      v8::Local<v8::Value> argv[1] = {
        err
      };
      callback->Call(1, argv, async_resource);
      ...
      if (!callbackFired) {
        v8::Local<v8::Object> err = Nan::Error("Method createGlob has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("ConfigIterator.createGlob").ToLocalChecked());
        v8::Local<v8::Value> argv[1] = {
          err
...
// start convert_from_v8 block
  ...
  String::Utf8Value regexp(info[2]->ToString());
  // malloc with one extra byte so we can add the terminating null character C-strings expect:
  from_regexp = (const char *) malloc(regexp.length() + 1);
  // copy the characters from the nodejs string into our C-string (used instead of strdup or strcpy because nulls in
  // the middle of strings are valid coming from nodejs):
...
// end convert_from_v8 block
  ...
  GitConfigMultivarIteratorNewWorker *worker = new GitConfigMultivarIteratorNewWorker(baton, callback);
        ...
        if (!info[2]->IsUndefined() && !info[2]->IsNull())
          worker->SaveToPersistent("regexp", info[2]->ToObject());
  ...
  // end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (baton->error->message) {
        ...
        err = Nan::Error("Method createMultivar has thrown an error.")->ToObject();
      }
      err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
      err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("ConfigIterator.createMultivar").ToLocalChecked());
      v8::Local<v8::Value> argv[1] = {
        err
      };
      callback->Call(1, argv, async_resource);
      ...
      if (!callbackFired) {
        v8::Local<v8::Object> err = Nan::Error("Method createMultivar has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        err->Set(Nan::New("errorFunction").ToLocalChecked(), Nan::New("ConfigIterator.createMultivar").ToLocalChecked());
        v8::Local<v8::Value> argv[1] = {
          err
...
*from_entry = Nan::ObjectWrap::Unwrap<GitConfigEntry>(info[0]->ToObject())->GetValue();
// end convert_from_v8 block
