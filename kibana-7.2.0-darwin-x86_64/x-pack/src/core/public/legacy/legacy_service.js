...
class LegacyPlatformService {
    ...
    setup({ core }) {
        const { application, i18n, injectedMetadata, fatalErrors, notifications, http, basePath, uiSettings, chrome, } = core;
        ...
        injectedMetadata.getLegacyMetadata().nav.forEach((navLink) => application.registerLegacyApp({
            id: navLink.id,
            order: navLink.order,
            title: navLink.title,
