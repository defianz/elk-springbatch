...
class PluginsSystem {
     ...
     */
    uiPlugins() {
        const internal = new Map([...this.getTopologicallySortedPluginNames().keys()]
        ...
        const publicPlugins = new Map([...internal.entries()].map(([pluginName, plugin]) => [
            pluginName,
            lodash_1.pick(plugin, ['id', 'configPath', 'requiredPlugins', 'optionalPlugins']),
        ]));
