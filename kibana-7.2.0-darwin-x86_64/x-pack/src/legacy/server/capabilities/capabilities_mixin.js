...
const capabilities_route_1 = require("./capabilities_route");
...
async function capabilitiesMixin(kbnServer, server) {
    ...
    kbnServer.afterPluginsInit(async () => {
        const defaultCapabilities = merge_capabilities_1.mergeCapabilities(...(await Promise.all(kbnServer.pluginSpecs
        ...
        server.decorate('request', 'getCapabilities', function () {
            ...
            const navLinks = server.getUiNavLinks().reduce((acc, spec) => ({
                ...acc,
                [spec._id]: true,
            }), {});
            return resolve_capabilities_1.resolveCapabilities(this, modifiers, defaultCapabilities, { navLinks });
        });
        capabilities_route_1.registerCapabilitiesRoute(server, defaultCapabilities, modifiers);
    });
}
