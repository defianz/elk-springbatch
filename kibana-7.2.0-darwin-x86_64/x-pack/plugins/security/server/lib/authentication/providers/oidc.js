 ...
 */
function isOIDCIncomingRequest(request) {
    return ((request.payload != null && !!request.payload.iss) ||
        (request.query != null &&
            (!!request.query.iss ||
                !!request.query.code ||
                !!request.query.error)));
 ...
 */
class OIDCAuthenticationProvider extends base_1.BaseAuthenticationProvider {
    constructor(options, oidcOptions) {
        ...
        if (type_detect_1.default(oidcOptions.realm) !== 'string') {
    ...
    async authenticate(request, state) {
        ...
        let { authenticationResult, headerNotRecognized, } = await this.authenticateViaHeader(request);
     ...
     */
    async authenticateViaResponseUrl(request, sessionState) {
        ...
        const loginHint = (request.query && request.query.login_hint) ||
        ...
        try {
            ...
            const { access_token: accessToken, refresh_token: refreshToken, } = await this.options.client.callWithInternalUser('shield.oidcAuthenticate', {
                body: {
            ...
            return authentication_result_1.AuthenticationResult.redirectTo(stateRedirectURL, {
                accessToken,
                refreshToken,
            });
        }
     ...
     * @param params
     */
    async initiateOIDCAuthentication(request, params, sessionState) {
        ...
        try {
             ...
             */
            const oidcPrepareParams = sessionState && sessionState.nonce && sessionState.state
            ...
            const { state, nonce, redirect } = await this.options.client.callWithInternalUser('shield.oidcPrepare', {
            ...
            const redirectAfterLogin = `${request.getBasePath()}${'iss' in params ? '/' : request.url.path}`;
    ...
    async authenticateViaHeader(request) {
        ...
        try {
            ...
            return {
                authenticationResult: authentication_result_1.AuthenticationResult.succeeded(user),
            };
        }
        catch (err) {
            this.debug(`Failed to authenticate request via header: ${err.message}`);
            return {
                authenticationResult: authentication_result_1.AuthenticationResult.failed(err),
            };
        }
    }
    /**
    ...
    async authenticateViaRefreshToken(request, { refreshToken }) {
        ...
        try {
            ...
            if (errors_1.getErrorStatusCode(err) === 400) {
                if (can_redirect_request_1.canRedirectRequest(request)) {
                    this.debug('Both elasticsearch access and refresh tokens are expired. Re-initiating OpenID Connect authentication.');
                    return this.initiateOIDCAuthentication(request, { realm: this.realm });
                }
                return authentication_result_1.AuthenticationResult.failed(boom_1.default.badRequest('Both elasticsearch access and refresh tokens are expired.'));
            }
            return authentication_result_1.AuthenticationResult.failed(err);
        }
    }
    ...
    async deauthenticate(request, state) {
        ...
        try {
            ...
            const { redirect } = await this.options.client.callWithInternalUser('shield.oidcLogout', logoutBody);
            ...
            return deauthentication_result_1.DeauthenticationResult.redirectTo(`${this.options.basePath}/logged_out`);
        }
        catch (err) {
