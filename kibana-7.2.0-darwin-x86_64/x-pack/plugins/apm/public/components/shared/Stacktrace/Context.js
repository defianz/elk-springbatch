...
const LineHighlight = styled_components_1.default.div `
  ...
  pointer-events: none;
  background-color: ${polished_1.tint(0.1, eui_theme_light_json_1.default.euiColorWarning)};
`;
...
const LineNumber = styled_components_1.default.div `
  ...
  background-color: ${props => props.highlight ? polished_1.tint(0.1, eui_theme_light_json_1.default.euiColorWarning) : null};
...
function getStackframeLines(stackframe) {
    ...
    const preLines = elastic_idx_1.idx(stackframe, _ => _.context.pre) || [];
    const postLines = elastic_idx_1.idx(stackframe, _ => _.context.post) || [];
    return [...preLines, line, ...postLines];
}
function getStartLineNumber(stackframe) {
    const preLines = lodash_1.size(elastic_idx_1.idx(stackframe, _ => _.context.pre) || []);
...
function Context({ stackframe, codeLanguage, isLibraryFrame }) {
    ...
    const highlightedLineIndex = lodash_1.size(elastic_idx_1.idx(stackframe, _ => _.context.pre) || []);
