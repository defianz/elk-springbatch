...
class Waterfall extends react_1.Component {
    constructor() {
        ...
        this.renderWaterfallItem = (item) => {
            ...
            const errorCount = item.docType === 'transaction'
                ? waterfall.errorCountByTransactionId[item.transaction.transaction.id]
                : 0;
            return (react_1.default.createElement(WaterfallItem_1.WaterfallItem, { key: item.id, timelineMargins: TIMELINE_MARGINS, color: serviceColors[item.serviceName], item: item, totalDuration: waterfall.duration, isSelected: item.id === urlParams.waterfallItemId, errorCount: errorCount, onClick: () => this.onOpenFlyout(item) }));
        };
        this.getFlyOut = () => {
            ...
            switch (currentItem.docType) {
                ...
                case 'transaction':
                    return (react_1.default.createElement(TransactionFlyout_1.TransactionFlyout, { transaction: currentItem.transaction, onClose: this.onCloseFlyout, traceRootDuration: waterfall.traceRootDuration, errorCount: currentItem.errorCount }));
                default:
                    return null;
            }
        };
