...
function PrefixIcon({ item }) {
    ...
    if (agent_name_1.isRumAgentName(item.transaction.agent.name)) {
...
const SpanActionToolTip = ({ item, children }) => {
    if (item && item.docType === 'span') {
        return (react_1.default.createElement(eui_1.EuiToolTip, { content: `${item.span.span.subtype}.${item.span.span.action}` },
...
function WaterfallItem({ timelineMargins, totalDuration, item, color, isSelected, errorCount, onClick }) {
    if (!totalDuration) {
        return null;
    }
    ...
    return (react_1.default.createElement(Container, { type: item.docType, timelineMargins: timelineMargins, isSelected: isSelected, onClick: onClick },
        ...
        , { style: { minWidth: `${Math.max(100 - left, 0)}%` } },
            react_1.default.createElement(SpanActionToolTip, { item: item },
            ...
            errorCount > 0 && item.docType === 'transaction' ? (react_1.default.createElement(ErrorCountBadge_1.ErrorCountBadge, { errorCount: errorCount, transaction: item.transaction })) : null,
