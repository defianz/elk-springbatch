...
function useTransactionOverviewCharts(urlParams) {
    ...
    const { data, error, status } = useFetcher_1.useFetcher(() => {
        if (serviceName && start && end) {
            return transaction_groups_1.loadTransactionOverviewCharts({
                serviceName,
                start,
                end,
                transactionType,
...
exports.useTransactionOverviewCharts = useTransactionOverviewCharts;
