...
function timeseriesTransformer({ timeseriesResponse, bucketSize }) {
    ...
    const overallAvgDuration = elastic_idx_1.idx(aggs, _ => _.overall_avg_duration.value);
    const responseTimeBuckets = elastic_idx_1.idx(aggs, _ => _.response_times.buckets);
    const { avg, p95, p99 } = getResponseTime(responseTimeBuckets);
    const transactionResultBuckets = elastic_idx_1.idx(aggs, _ => _.transaction_results.buckets);
