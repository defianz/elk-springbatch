...
class NavControlPopover extends react_1.Component {
    ...
    render() {
        ...
        if (this.state.spaces.length < 2) {
            element = react_1.default.createElement(spaces_description_1.SpacesDescription, { onManageSpacesClick: this.toggleSpaceSelector });
        }
        else {
            element = (react_1.default.createElement(spaces_menu_1.SpacesMenu, { spaces: this.state.spaces, onSelectSpace: this.onSelectSpace, onManageSpacesClick: this.toggleSpaceSelector }));
        }
        return (
        // @ts-ignore repositionOnScroll doesn't exist on EuiPopover
        react_1.default.createElement(eui_1.EuiPopover, { id: 'spcMenuPopover', "data-test-subj": `spacesNavSelector`, button: button, isOpen: this.state.showSpaceSelector, closePopover: this.closeSpaceSelector, anchorPosition: this.props.anchorPosition, panelPaddingSize: "none", repositionOnScroll: true, withTitle: this.props.anchorPosition.includes('down'), ownFocus: true }, element));
    }
    async loadSpaces() {
