...
var _handle_es_error = _interopRequireDefault(require("../../../lib/handle_es_error"));
  ...
  server.route({
    ...
    handler: async function (req) {
      ...
      try {
        ...
        throw (0, _handle_es_error.default)(error);
      }
    }
  });
}
...
function getBody({
  ...
  const executionHint = 'map'; // Helps keep the number of buckets that need to be tracked at the shard level contained in case
  // this is a high cardinality field

  const terminateAfter = 100000; // We don't care about the accuracy of the counts, just the content of the terms, so this reduces
