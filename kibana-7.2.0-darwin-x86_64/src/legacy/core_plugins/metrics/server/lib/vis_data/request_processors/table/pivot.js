  ...
  return next => doc => {
    ...
    if (panel.pivot_id) {
      ...
      if (sort) {
        ...
        } else if (metric && _basic_aggs.default.includes(metric.type)) {
          const sortAggKey = `${metric.id}-SORT`;
          const fn = _bucket_transform.default[metric.type];
          const bucketPath = (0, _get_buckets_path.default)(metric.id, series.metrics).replace(metric.id, sortAggKey);
          (0, _lodash.set)(doc, `aggs.pivot.terms.order`, {
