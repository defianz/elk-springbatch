...
var _get_split_colors = _interopRequireDefault(require("./get_split_colors"));
...
function getSplits(resp, panel, series, meta) {
  ...
  if (buckets) {
    if (Array.isArray(buckets)) {
      ...
      const colors = (0, _get_split_colors.default)(series.color, size, series.split_color_mode);
  ...
  return [{
    id: series.id,
    label: series.label || (0, _calculate_label.default)(metric, series.metrics),
    color: color.string(),
    ...mergeObj,
    meta
  }];
