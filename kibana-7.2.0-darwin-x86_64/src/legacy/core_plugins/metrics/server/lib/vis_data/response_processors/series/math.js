...
var _get_default_decoration = _interopRequireDefault(require("../../helpers/get_default_decoration"));
...
var _map_bucket = _interopRequireDefault(require("../../helpers/map_bucket"));
...
function mathAgg(resp, panel, series, meta) {
  return next => results => {
    ...
    const decoration = (0, _get_default_decoration.default)(series);
    const splits = (0, _get_splits.default)(resp, panel, series, meta);
        ...
        const splitData = mathMetric.variables.reduce((acc, v) => {
          ...
          if (/_bucket$/.test(metric.type)) {
            acc[v.name] = split.timeseries.buckets.map(bucket => {
              return [bucket.key, (0, _get_sibling_agg_value.default)(split, metric)];
            });
          } else {
            ...
            acc[v.name] = split.timeseries.buckets.map((0, _map_bucket.default)(m));
          }
