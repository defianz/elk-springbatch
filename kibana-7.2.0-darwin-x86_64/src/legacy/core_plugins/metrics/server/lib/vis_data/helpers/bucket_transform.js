...
function extendStatsBucket(bucket, metrics) {
  const bucketsPath = 'timeseries>' + (0, _get_buckets_path.default)(bucket.field, metrics);
  const body = {
...
var _default = {
  ...
  derivative: (bucket, metrics, bucketSize) => {
    ...
    const body = {
      derivative: {
        buckets_path: (0, _get_buckets_path.default)(bucket.field, metrics),
        gap_policy: 'skip',
        // seems sane
        unit: bucketSize
      }
  ...
  moving_average: (bucket, metrics) => {
    ...
    if (bucket.settings) {
      body.moving_avg.settings = (0, _parse_settings.default)(bucket.settings);
    }
    ...
    const body = {
      bucket_script: {
        buckets_path: bucket.variables.reduce((acc, row) => {
          acc[row.name] = (0, _get_buckets_path.default)(row.field, metrics);
          return acc;
        }, {}),
        script: {
  ...
  positive_only: (bucket, metrics) => {
    ...
    const body = {
      bucket_script: {
        buckets_path: {
          value: (0, _get_buckets_path.default)(bucket.field, metrics)
        },
        script: {
