...
function interpreterProvider(config) {
  ...
  async function invokeChain(chainArr, context) {
    ...
    const chain = (0, _lodash.default)(chainArr);
  ...
  async function resolveArgs(fnDef, context, argAsts) {
    ...
    const dealiasedArgAsts = (0, _lodash2.reduce)(argAsts, (argAsts, argAst, argName) => {
    ...
    (0, _lodash2.each)(argDefs, argDef => {
      const {
        aliases,
        default: argDefault,
        name: argName,
    ...
    const argAstsWithDefaults = (0, _lodash2.reduce)(argDefs, (argAsts, argDef, argName) => {
      if (typeof argAsts[argName] === 'undefined' && typeof argDef.default !== 'undefined') {
        argAsts[argName] = [(0, _common.fromExpression)(argDef.default, 'argument')];
      }
    ...
    const resolveArgFns = (0, _lodash2.mapValues)(argAstsWithDefaults, (argAsts, argName) => {
      return argAsts.map(argAst => {
        return async (ctx = context) => {
          const newContext = await interpret(argAst, ctx); // This is why when any sub-expression errors, the entire thing errors
    ...
    const argNames = (0, _lodash2.keys)(resolveArgFns); // Actually resolve unless the argument definition says not to
    ...
    const resolvedMultiArgs = (0, _lodash2.zipObject)(argNames, resolvedArgValues); // Just return the last unless the argument definition allows multiple

    const resolvedArgs = (0, _lodash2.mapValues)(resolvedMultiArgs, (argValues, argName) => {
      if (argDefs[argName].multi) return argValues;
      return (0, _lodash2.last)(argValues);
