...
exports.injectNestedDependencies = injectNestedDependencies;
...
function getObjectReferencesToFetch(savedObjectsMap) {
  ...
  for (const savedObject of savedObjectsMap.values()) {
    for (const {
      type,
      id
    } of savedObject.references || []) {
      if (!savedObjectsMap.has(`${type}:${id}`)) {
        objectsToFetch.set(`${type}:${id}`, {
          type,
          id
        });
      }
...
async function injectNestedDependencies(savedObjects, savedObjectsClient) {
  ...
  while (objectsToFetch.length > 0) {
    const bulkGetResponse = await savedObjectsClient.bulkGet(objectsToFetch); // Check for errors
