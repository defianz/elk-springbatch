...
async function getNonExistingReferenceAsKeys(savedObjects, savedObjectsClient) {
  ...
  const bulkGetResponse = await savedObjectsClient.bulkGet(bulkGetOpts); // Error handling
...
async function validateReferences(savedObjects, savedObjectsClient) {
  const errorMap = {};
  const nonExistingReferenceKeys = await getNonExistingReferenceAsKeys(savedObjects, savedObjectsClient); // Filter out objects with missing references, add to error object
